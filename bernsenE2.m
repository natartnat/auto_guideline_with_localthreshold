...
function [output,mask,globalMask,localmask] = bernsenE2(image, varargin)
% Initialization
...
% [localMin,localMax] = bernsenTfilter(image,window(1));

localMax = maxfilt2(image, window);
localMin = minfilt2(image, window);
T = 0.5*(localMin + localMax);
% localMaxImage = colfilt(image, [window window], 'sliding', @max);
% localMinImage = colfilt(image, [window window], 'sliding', @min);
% Local contrast
