%NEW local thresholding.
%   BW = NEWLOC(IMAGE) performs local thresholding of a two-dimensional 
%   array IMAGE with Sauvola algorithm.
%      
%   BW = NEWLOC(IMAGE, [M N], THRESHOLD, PADDING) performs local 
%   thresholding with M-by-N neighbourhood (default is 3-by-3) and 
%   threshold THRESHOLD between 0 and 1 (default is 0.06). 
%   To deal with border pixels the image is padded with one of 
%   PADARRAY options (default is 'replicate').
%       
%   Example
%   -------
%       imshow(NEWLOC(imread('eight.tif'), [150 150]));
%
...
function output=NEWLOCAL(image, varargin)
% Initialization
...
J = integralImage(image);
JT = J(2:end,2:end);
absMean = abs(JT - mean);
meanDev = averagefilter(absMean, window, padding);
% Sauvola
% R = max(deviation(:));
threshold = mean.*(1 + k * ((meanDev ./ (1 - meanDev)) -1));
